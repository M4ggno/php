...
where nome like '%CESS%IA';

select * from fornecedor f
where cidade like '_____%E';

select c. * from cliente c, produto p, venda v, fornecedor f 
where c.codcli = v.codcli and v.codprod = p.codprod and f.codfor = p.fornecedor 
and p.preco < 300 and p.nome like '______%' and f.cidade like '_A%CO';

select codcli, nome from cliente
where endereco is null;

select c.nome from cliente c, venda v 
where c.codcli = v.codcli and endereco is null order by c.nome desc;

select p.nome produto, p.preco, f.nome fornecedor
from produto p, fornecedor f
where f.codfor =p.fornecedor
and p.preco = (select max(preco) from produto);

select c.* from cliente c, venda v, produto p, fornecedor f
where c.codcli = v.codcli and v.codprod;

select c.nome cliente, count(*) quant_produtos, sum(p.preco) valor_total
from cliente c, venda v, produto p 
where c.codcli = v.codcli and v.codprod = p.codprod
group by c.codcli, c.nome;

select c.codcli, c.nome cliente, count(*) quant_produtos, sum(p.preco) valor_total 
from cliente c, venda v, produto p 
where c.codcli = v.codcli and v.codprod = p.codprod
group by c.codcli, c.nome
order by c.codcli;
